...
func _init(propResource : PropResource, default_variant := "default"):
	...
	for variant in propResource.variants.keys():
		if propResource.variants[variant] != null:
			_variants[variant] = load(propResource.variants[variant])
		else:
			_variants[variant] = null
	
	for flip_variant in propResource.flip_aliases.keys():
		if propResource.flip_aliases[flip_variant] != null:
			_flip_aliases[flip_variant] = load(propResource.flip_aliases[flip_variant])
		else:
			pass
