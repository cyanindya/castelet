...
# - Early during runtime, this program will search for all CasteletPropResource-derived resource
#   files (.tres) for all data related to "stage props" (including puppets/actors).
#   Each of these .tres files will be instantiated into CasteletPropNode-class nodes and
...
func _prop_loader_callback(file_name : String):
	...
	# then check if it is a CasteletPropResource-type data. If it is, generate
	# an instance of CasteletPropNode that can be used in the Theater node later.
	#
	...
	if file_name.ends_with(".tres"):
		...
		if res is CasteletPropResource:
			var new_prop = CasteletPropNode.new(res)
			props[res.prop_id] = new_prop

		if res is CasteletAudioListResource:
